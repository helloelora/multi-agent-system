...
Provides a web dashboard at localhost:8080 and a JSON API for live state.

Usage:
    python server.py

Endpoints:
    GET /             - HTML dashboard with auto-refresh and canvas charts
    GET /api/state    - JSON snapshot of the current simulation state
    POST /api/start   - Start the simulation
    POST /api/stop    - Pause the simulation
    POST /api/reset   - Reset the simulation to initial state
"""
...
# Initialize pygame in headless mode before importing model
import os
...
import pygame
pygame.init()
# Create a tiny dummy display so sprites can render
...
def _simulation_loop():
    """Background thread that steps the model periodically."""
    global _running
    while _running:
        with _lock:
...
def _get_state():
    """Build a JSON-serializable snapshot of the model."""
    with _lock:
        m = _model
        # Waste positions
        waste_list = []
