...
def _draw_mech_robot(pal, size=32, frame=0, facing="right"):
    """Heavy industrial mech: layered armor, wide visor, stomping gait, radar antenna."""
    s = _surf(size, size)
    cx = size // 2
...
def _draw_ninja_robot(pal, size=32, frame=0, facing="right"):
    """Sleek ninja: angular, scarf, blade with gleam, after-image trail."""
    s = _surf(size, size)
    cx = size // 2
...
def _draw_tank_robot(pal, size=32, frame=0, facing="right"):
    """Massive tank: wide body, animated treads, dual exhaust, turret cannon."""
    s = _surf(size, size)
    cx = size // 2
...
def _draw_drone_robot(pal, size=32, frame=0, facing="right"):
    """Hovering drone: crossed propellers, scanning beam, side thrusters with flame."""
    s = _surf(size, size)
    cx = size // 2
...
def _draw_knight_robot(pal, size=32, frame=0, facing="right"):
    """Noble knight: T-visor helmet, flowing plume, emblem shield, gleaming sword, cape."""
    s = _surf(size, size)
    cx = size // 2
...
def _draw_claude_robot(pal, size=32, frame=0, facing="right"):
    """Claude Code CLI-style robot: warm, friendly, round head, expressive eyes, terminal chest."""
    s = _surf(size, size)
    cx = size // 2
...
def _make_zone1_tile(size, parity):
    """Lush grasslands: varied greens, mushrooms, wildflowers, moss, puddles, bugs."""
...
def _make_zone2_tile(size, parity):
    """Desert: sand ripples, bones, rock cairns, cracks, cactus, tumbleweed."""
    s = pygame.Surface((size, size))
    wc = WORLD_COLORS
...
def _make_zone3_tile(size, parity):
    """Volcano: obsidian rock, lava cracks, ash, embers, jagged formations, toxic vents."""
    s = pygame.Surface((size, size))
    wc = WORLD_COLORS
...
def _make_dark_zone1_tile(size, parity):
    """Dark teal terminal tile: circuit-board traces, hex grid pattern."""
    s = pygame.Surface((size, size))
...
def _make_dark_zone2_tile(size, parity):
    """Dark amber terminal tile: warning data streams, matrix-style dots."""
    s = pygame.Surface((size, size))
    dc = DARK_WORLD_COLORS
...
def _make_dark_zone3_tile(size, parity):
    """Dark red terminal tile: red alert patterns, danger hash marks, pulsing corners."""
    s = pygame.Surface((size, size))
    dc = DARK_WORLD_COLORS
...
class ParticleSystem:
    ...
    def emit_burst(self, x, y, color, count=12):
        """Ring burst: particles radiate outward in evenly spaced directions."""
