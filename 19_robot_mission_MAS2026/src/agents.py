...
class RobotAgent:
    ...
    def __init__(self, agent_id, x, y):
        ...
        self.inventory_uids = []   # parallel list of Waste.uid for block identity tracking
        self.energy = AGENT_MAX_ENERGY if ENERGY_ENABLED else AGENT_MAX_ENERGY
        self.mailbox = []          # incoming messages for communication
        self.pickup_self = 0       # pickups of blocks first discovered by this robot
        self.pickup_alerted = 0    # pickups of blocks first learned from another robot's message
        self.knowledge = {
            "pos": (x, y),
            "prev_pos": (x, y),
            "percepts": {},
    ...
    def step_agent(self, model):
        ...
        if action == ACTION_PICK_UP and len(self.inventory) > prev_inv_size:
            ...
            if picked_uid is not None and picked_uid not in counted:
                counted.add(picked_uid)
                entry = self.knowledge["known_waste"].get(self.pos)
                if entry and entry.get("source") == "alerted":
                    self.pickup_alerted += 1
                else:
                    self.pickup_self += 1
        ...
        for pos, contents in percepts.items():
            if contents.get("waste"):
                for waste_type in contents["waste"]:
                    existing = self.knowledge["known_waste"].get(pos)
                    source = existing["source"] if existing and "source" in existing else "self"
            ...
            if msg_type == "waste_picked":
                ...
                if wpos is not None and wtype:
                    ...
                    if msg_type == "need_pickup" and wtype == self.target_waste:
                        ...
                        ttl = KNOWLEDGE_WASTE_TTL
                    key = tuple(wpos)
                    existing = self.knowledge["known_waste"].get(key)
                    source = existing["source"] if existing and "source" in existing else "alerted"
                    self.knowledge["known_waste"][key] = {
                        "type": wtype,
                        "ttl": ttl,
                        "source": source,
                    }
