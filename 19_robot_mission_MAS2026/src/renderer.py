...
class Renderer:
    ...
    def _draw_sidebar(self, model):
        ...
        y += 12
        y = self._sb_draw_pickup_stats(content_surface, model, 0, y, sb_w)
        y += 12
    ...
    def _measure_sidebar_content(self, model, sb_w):
        ...
        y += 12
        # Pickup stats bar chart (title + legend + chart)
        y += 20 + 16 + 160
        y += 12
        return y
    ...
    def _sb_draw_pickup_stats(self, surf, model, x, y, w):
        ...
        # Legend row: two swatches + labels
        sw = 10
        legend_color = (200, 200, 210)
        lx = x + 4
        pygame.draw.rect(surf, legend_color, (lx, y + 2, sw, sw))
        self._surf_text(surf, "found", lx + sw + 4, y, self.font_small, legend_color)
        lx2 = lx + sw + 4 + 42
        pygame.draw.rect(surf, legend_color, (lx2, y + 2, sw, sw), 1)
        self._surf_text(surf, "alerted", lx2 + sw + 4, y, self.font_small, legend_color)
        y += 16

        chart_h = 160
        pygame.draw.rect(surf, (18, 18, 28), (x, y, w, chart_h), border_radius=4)
        pygame.draw.rect(surf, (45, 45, 58), (x, y, w, chart_h), 1, border_radius=4)

        order = {"green": 0, "yellow": 1, "red": 2}
        robots = [r for r in model.robots if r.robot_type in order]
        robots.sort(key=lambda r: (order[r.robot_type], r.agent_id))
        if not robots:
            return y + chart_h

        max_val = max(
            (max(getattr(r, "pickup_self", 0), getattr(r, "pickup_alerted", 0)) for r in robots),
            default=0,
        )
        ...
        margin_top = 14
        margin_bottom = 22
        label_y = y + chart_h - margin_bottom + 4
        plot_h = chart_h - margin_top - margin_bottom
        plot_y = y + margin_top

        n = len(robots)
        group_gap = 10
        group_w = (w - group_gap * (n + 1)) // n
        bar_gap = 4
        bar_w = (group_w - bar_gap) // 2

        for i, robot in enumerate(robots):
            gx = x + group_gap + i * (group_w + group_gap)
            base_color = _ROBOT_COLORS.get(robot.robot_type, (180, 180, 180))
            self_val = getattr(robot, "pickup_self", 0)
            alert_val = getattr(robot, "pickup_alerted", 0)

            self_h = int(plot_h * self_val / max_val)
            alert_h = int(plot_h * alert_val / max_val)

            sx = gx
            ax = gx + bar_w + bar_gap

            # Solid filled bar = self-found
            if self_h > 0:
                pygame.draw.rect(surf, base_color,
                                 (sx, plot_y + plot_h - self_h, bar_w, self_h), border_radius=2)
            # Outlined bar = alerted
            if alert_h > 0:
                pygame.draw.rect(surf, base_color,
                                 (ax, plot_y + plot_h - alert_h, bar_w, alert_h), 2, border_radius=2)

            # Value labels above bars (only when > 0)
            if self_val > 0:
                self._surf_text(surf, str(self_val), sx, plot_y + plot_h - self_h - 12,
                                self.font_small, base_color)
            if alert_val > 0:
                self._surf_text(surf, str(alert_val), ax, plot_y + plot_h - alert_h - 12,
                                self.font_small, base_color)

            # Robot label under bars
            label = f"{robot.robot_type[0].upper()}{robot.agent_id}"
            self._surf_text(surf, label, gx + group_w // 2 - 8, label_y,
                            self.font, base_color)
